...
qtableBean board=db.getBoard(id, true);
...
<body>
        ...
        <table border="1" class="table">
            ...
            <tr>
                <th>첨부파일</th>
                <td colspan="3"><input type="file" name="q_fname"><%=q_fname%></td>	
            </tr>
            
            
        </table>
        <div class="section3">
			<input type="button" value="수정" onclick="location.href='edit.jsp?q_id=<%=q_id %>&pageNum=<%=pageNum %>'"/> 
			<input type="button" value="삭제" onclick="location.href='delete.jsp?q_id=<%=q_id %>&pageNum=<%=pageNum %>'"/>
           <input type="button" value="답변글" onclick="location.href='write.jsp?q_id=<%=q_id %>&pageNum=<%=pageNum %>'"/>
           <input type="button" value="목록" onclick="location.href='qna.jsp?pageNum=<%=pageNum %>'" ></a>
            </div>     
       </div>
    <!-- 푸터 -->
	<div id="footer">
